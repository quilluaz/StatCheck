...
const TimeSlotComponent = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [scheduleResponse, subjectResponse, timeSlotResponse] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedTimeSlot = await axios.get(`${API_URL}/getById/${editId || response.data.timeSlotId}`);
      ...
      setError(editId 
        ? `Failed to update time slot: ${error.response?.data?.message || error.message}` 
        : `Failed to create time slot: ${error.response?.data?.message || error.message}`
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDelete = async (timeSlotId) => {
    ...
    try {
      ...
      setTimeSlots(timeSlots.filter(timeSlot => timeSlot.timeSlotId !== timeSlotId));
    } catch (error) {
      setError(`Failed to delete time slot: ${error.response?.data?.message || error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <Grid item xs={12} sm={6}>
              ...
              >
                {schedules.map((schedule) => (
                  <MenuItem key={schedule.scheduleId} value={schedule.scheduleId?.toString() || ''}>
          ...
          <TableBody>
            {timeSlots.map((timeSlot) => (
              <TableRow key={timeSlot.timeSlotId}>
                ...
                <TableCell>{timeSlot.schedule ? `${timeSlot.schedule.dayOfWeek} - ${timeSlot.schedule.roomID}` : 'N/A'}</TableCell>
                <TableCell>{timeSlot.subject ? `${timeSlot.subject.subjectName} - ${timeSlot.subject.section}` : 'N/A'}</TableCell>
                <TableCell>
                  <IconButton onClick={() => handleEdit(timeSlot)} color="primary">
                    <EditIcon />
                  </IconButton>
                  <IconButton onClick={() => handleDelete(timeSlot.timeSlotId)} color="error">
                    <DeleteIcon />
                  </IconButton>
                </TableCell>
              </TableRow>
